...
public class IntegerAsString {
	...
	public static String add(String num1, String num2) {
		...
		for (int i = 0; i < al; i++) {
			...
			int col = Integer.valueOf(a.substring(posa, posa + 1))
					+ (posb >= 0 ? Integer.valueOf(b.substring(posb, posb + 1)) : 0);
	...
	public int[] digits() {
		...
		for (int i = 0; i < this.length(); i++)
			res[i] = Integer.valueOf(this.num.substring(i, i + 1));
		return res;
	}

	public boolean isGreaterThan(IntegerAsString x) {
		if (this.length() > x.length())
			return true;
		if (this.length() < x.length())
			return false;
		for (int i = 0; i < this.length(); i++)
			if ((int) this.num.charAt(i) > (int) x.num.charAt(i))
				return true;
		return false;
	}

	public boolean isLessThan(IntegerAsString x) {
		if (this.length() < x.length())
			return true;
		if (this.length() > x.length())
			return false;
	...
	public void increment() {
		this.num = add(this.num, "1");
	}

	public int intValue() {
		if (this.isGreaterThan(IntegerAsString.valueOf(Integer.MAX_VALUE)))
			return 0;
		int res = 0;
